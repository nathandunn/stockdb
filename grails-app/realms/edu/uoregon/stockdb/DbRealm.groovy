...
class DbRealm {
    ...
    def authenticate(authToken) {
        ...
        // exception.
        def user = Researcher.findByUsername(username)
        if (!user) {
    ...
    def hasRole(principal, roleName) {
        def roles = Researcher.withCriteria {
            roles {
                eq("name", roleName)
            }
    ...
    def isPermitted(principal, requiredPermission) {
        ...
        def user = Researcher.findByUsername(principal)
        ...
        def results = Researcher.executeQuery("select distinct p from Researcher as user join user.roles as role join role.permissions as p where user.username = '$principal'")
