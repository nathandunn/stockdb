...
<html>
	...
	<body>
		<a href="#list-measuredValue" class="skip" tabindex="-1"><g:message code="default.link.skip.label" default="Skip to content&hellip;"/></a>
			...
			<table>
				...
				<g:each in="${measuredValueInstanceList}" status="i" var="measuredValueInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${measuredValueInstance.id}">${fieldValue(bean: measuredValueInstance, field: "name")}</g:link></td>
					
						<td>${fieldValue(bean: measuredValueInstance, field: "value")}</td>
					
						<td>${fieldValue(bean: measuredValueInstance, field: "experiment")}</td>
