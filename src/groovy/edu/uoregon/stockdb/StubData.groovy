...
//    }
    ...
    Map<String, String> stubStickleback1() {

        Map<String, String> strainMap = new HashMap<>()
        // Strain[0],Box[1],Position[2],Genus[3],Species[4-addNote],
        // Ana/ aerobic[5],Expt date / name[6],isolation date[7], fish age[8],Med[9],
        // FishOrigin[10],Origin[11],Anatomy of Origin[12],Size[13],
        // Color[14],Edge[15],Raised?[16],Shape[17],Notes?[18]
        ...
        csvReader.eachLine { tokens ->
            ...
            if (tokens.size() < 19) return
            if (tokens[0].trim().size() == 0) return
            if (!tokens[6] || !tokens[7]) {
                println "ignoring Row - ${rowIndex} - strain ${tokens[0]}"
                return
            }

            String strainName = generateSticklebackStrainName()
            strainMap.put(tokens[0], strainName)

            Strain strain = new Strain(name: strainName)
            strain.notes += tokens[0] ? "Old Strain Index: ${tokens[0]?.trim()}\n" : ""
            strain.notes += tokens[4] ? "Strain Species: ${tokens[4]?.trim()}\n" : ""
            strain.notes += tokens[18] ? "Strain Note: ${tokens[18]?.trim()}\n" : ""
            ...
            String media = tokens[9]?.trim()
            Date isolationDate = tokens[7] ? Date.parse("MMddyy", tokens[7]) : null
            println "isolation date ${rowIndex} -> ${isolationDate}"
            IsolateCondition isolateCondition = IsolateCondition.findOrSaveByMediaAndOxygenConditionAndIsolatedByAndIsolatedWhen(media, oxygenCondition, kat,isolationDate)

//            isolateCondition.isolatedWhen = isolationDate
            strain.isolateCondition = isolateCondition
            isolateCondition.addToStrains(strain)
            isolateCondition.save(flush: true, failOnError: true)
...
//            }
    ...
    String generateSticklebackStrainName() {
        ...
        String maxStrainName = maxStrain?.name?.substring(4)
        if (!maxStrainName) {
            maxStrainName = "0000"
        }
...
//        String returnString = "ZOR" + String.pa(maxInteger+1).
    ...
    def stubStickleback1Experiments(Map<String, String> strainMap) {
        ...
        csvReader.eachLine { tokens ->
            ...
            println "strainMap size: ${strainMap.size()}"

            println "input = [${tokens[0]?.trim()}]-> ${strainMap.get(tokens[0]?.trim())}"
            Strain strain = Strain.findByName(strainMap.get(tokens[0]?.trim()))
            println "strain found: ${strain}"
            String experimentDateString = tokens[6]?.trim()
            println "looking for experiment ${experimentDateString}"
            Experiment experiment = Experiment.findByName(experimentDateString)
            Integer numTokens = tokens.size()

            String sizeValue = tokens[13]?.trim()
            ...
            String colorValue = tokens[14]?.trim()
            ...
            String edgeValue = tokens[15]?.trim()
            ...
            String raisedValue = tokens[16]?.trim()
            ...
            String shapeValue = tokens[17]?.trim()
